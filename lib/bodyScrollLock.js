(function (global, factory) {
  ...
  function getScrollParent(node, root, diff) {
    if (!node || !root) {
      return null;
    }
    ...
    if (node.clientHeight > 0 && node.scrollHeight > node.clientHeight && (node.scrollTop > 0 || diff < 0) && (!isTargetElementTotallyScrolled(node) || diff > 0)) {
      return node;
    } else {
  ...
  var handleScroll = function handleScroll(event, targetElement) {
    ...
    var scrollableTarget = getScrollParent(event.target, targetElement, clientY);
